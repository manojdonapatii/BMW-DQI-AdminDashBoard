                            ...
                            <table id="datatable-buttons" class="table table-striped table-bordered" cellspacing="0" width="100%">
                               <thead>
                                <tr>
                                    ...
                                    <th>Dealer comment</th>
                                    <th>Photographic evidences</th>
                                </tr>
                                </thead>
                                <tbody>
                                <c:forEach var="hBean" items="${questionsList}" varStatus="status">
                                <tr>
                                   	 <td>${hBean.number }</td>
                                      <td>${hBean.standard }</td>
                                      <td>${hBean.essential }</td>
                                      <td>${hBean.requirement }</td>
                                     <td>${hBean.auditor_comment }</td> 
                                       <td>${hBean.dealer_comment }</td>
                                      
                                       <td> <c:forEach var="count" items="${ImageList.get(status.index)}"><a target="_blank" href="<%=UI_PATH %>uploads/questionnairy_images/${year }/${phase }/${count.reference_image }"><img src="<%=UI_PATH %>uploads/questionnairy_images/${year }/${phase }/${count.reference_image }"></a></c:forEach></td>     
