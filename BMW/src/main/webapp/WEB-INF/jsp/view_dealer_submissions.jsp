    ...
    <div id="wrapper">
      ...
      <div class="content-page">
                           ...
                           <div class="row">
                            ...
                            <table id="datatable-buttons" class="table table-striped table-bordered" cellspacing="0" width="100%">
                                ...
                                <tr>
                                    ...
                                    <th>Year</th>
                                    <th>Phase</th>
                                    <th>BMW/Sales-Rank</th>
                                    <th>BMW/Service-Rank</th>
                                    <th>MINI/Sales-Rank</th>
                                    <th>Action</th>
                                </tr>
                                </thead>
                                <tbody>
                                <c:forEach var="uBean" items="${dealerCompletedList}">
                                <tr>
                                    <td>${uBean.dealership_name }</td>
                                      <td>${uBean.year }</td>
                                      <td>${uBean.quarter }</td>
                                      <td>${uBean.BMW_sales_rank }</td>
                                       <td>${uBean.BMW_service_rank }</td>
                                        <td>${uBean.MINI_sales_rank }</td>
                                    <td><a  href="<%=dashboardURL%>publish_score/${uBean.dealership_id}/${uBean.year}/${uBean.quarter }">
                                        Publish Score
                                        </a> 
                                        &nbsp;|&nbsp;
                                     <a target="_blank" href="<%=dashboardURL%>getDealerSectionScore/${uBean.dealership_id}/${uBean.year}/${uBean.quarter }">
                                        Review Score
                                        </a>
                                         &nbsp;|&nbsp;
                                     <a target="_blank" href="<%=dashboardURL%>dealer_exception_/${uBean.dealership_id}/${uBean.quarter }/${uBean.year}">
                                        Dealer Submissions
                                        </a>
        ...
        function callme()
        {
        	...
        	var phase = $("#phase").val();
        	var year = $("#year").val();
        	location.replace("<%=dashboardURL %>viewDealerSubmissions/"+phase+"/"+year); 
        }
            </script>
